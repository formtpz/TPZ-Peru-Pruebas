...
# ------------------------------------------------------------
# Funciones auxiliares (idénticas a las de Descarga_Partidas_Juridico)
# ------------------------------------------------------------
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            execute(
                ...
                """,
                params=[
                    marca, usuario, nombre, puesto, supervisor, "Control de Calidad Precampo Jurídico",
...
def CC_Precampo_Juridico(usuario, puesto):
    ...
    procesos_btn = placeholder2.button("Procesos", key="procesos_cc_precampo")
    placeholder3 = st.sidebar.empty()
    historial_btn = placeholder3.button("Historial", key="historial_cc_precampo")
    placeholder4 = st.sidebar.empty()
    capacitacion_btn = placeholder4.button("Capacitaciones", key="capacitacion_cc_precampo")
    placeholder5 = st.sidebar.empty()
    otros_btn = placeholder5.button("Otros Registros", key="otros_cc_precampo")
    placeholder6 = st.sidebar.empty()
    bonos_btn = placeholder6.button("Bonos y Horas Extras", key="bonos_cc_precampo")
    placeholder7 = st.sidebar.empty()
    salir_btn = placeholder7.button("Salir", key="salir_cc_precampo")
    ...
    modo_carga = placeholder_modo.radio(
        ...
        key="modo_cc_precampo"
    )

    # ----- Título -----
    placeholder_titulo = st.empty()
    placeholder_titulo.title("Control de Calidad Precampo Jurídico")
    ...
    if modo_carga == "📝 Carga Manual (Formulario)":
        ...
        fecha = placeholder_fecha.date_input("Fecha", value=default_date, key="fecha_cc_precampo")
        ...
        partida = placeholder_partida.text_input("Número de Partida", value="N/A", max_chars=60, key="partida_cc_precampo")
        ...
        horas = placeholder_horas.number_input(
            ...
            min_value=0.0,
            key="horas_cc_precampo"
        )

        placeholder_obs = st.empty()
        observaciones = placeholder_obs.text_input("Observaciones", value="N/A", max_chars=60, key="obs_cc_precampo")

        placeholder_reporte = st.empty()
        reporte_btn = placeholder_reporte.button("Generar Reporte", key="reporte_cc_precampo")
    ...
    # ----- Modo masivo -----
        ...
        archivo_excel = placeholder_archivo.file_uploader(
            ...
            type=['xlsx', 'xls'],
            key="archivo_cc_precampo_masivo",
    ...
    if procesos_btn:
        ...
        # Inserción manual idéntica a Descarga Partidas, solo cambia el proceso
        marca = datetime.now(pytz.timezone('America/Guatemala')).strftime("%Y-%m-%d %H:%M:%S")
        usuario_activo = obtener_usuario_activo(usuario)
        if not usuario_activo:
            st.error("No se encontró un usuario activo para generar el reporte.")
