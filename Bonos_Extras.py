...
def Bonos_Extras(usuario, puesto):
    ...
    # -------------------------------------------------------------------
        ...
        personal_sel = ph_personal.selectbox("Personal", data_personal, key="personal_9")
        ...
        periodo_sel = ph_periodo.selectbox("Periodo de Bono", options=periodos, key="periodo_9")

        if personal_sel == "Todos":
            ph_bonos = st.empty()
            placeholders_contenido.append(ph_bonos)
            ph_bonos.subheader("Bonos")
            bonos_df = fetch_df(
            ...
            ph_extras.subheader("Horas Extra")
            extras_df = fetch_df(
    ...
    # -------------------------------------------------------------------
        ...
        if personal_sel == "Todos":
            ...
            ph_unidades.subheader("Unidades de Asignación")
    ...
    # -------------------------------------------------------------------
        ...
        ph_periodo = st.empty()
        placeholders_contenido.append(ph_periodo)
        periodo_sel = ph_periodo.selectbox("Periodo", options=periodos, key="periodo_bonos_9")
        ...
        ph_extras = st.empty()
        placeholders_contenido.append(ph_extras)
        ph_extras.subheader("Horas Extras")
        extras_df = fetch_df(
            "SELECT horas, marca, fecha, justificacion FROM extras WHERE nombre = %s AND tipo_reporte IN ('Extra','Horas Extra','Horas Extra Apoyo Otros Proyectos') AND fecha_bono = %s",
